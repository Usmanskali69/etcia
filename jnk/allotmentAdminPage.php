...
if ($own_row['applicationStatus'] == 'Seat Allocated' && $own_row['birthPlace'] != '' ) {
	echo '<script>alert("This student have already taken action.")</script>';
}
// mysqli_close($con);
?>
<!DOCTYPE html>
...
<body>
	<div id="header" class="navbar navbar-default navbar-fixed-top">
		<div class="navbar-header">
			...
			<a class="navbar-brand" href="index.php">
				<i class="fa fa-graduation-cap fa-lg"></i> PMSSS J&K Scholarships (Admin Page)
			</a>
		</div>
	...
	<br><br>
		...
		<div class="col-lg-offset-6 col-lg-6" style="margin-top:20px;">
			Application Status:<b> <?php echo $own_row["applicationStatus"]; ?></b>&nbsp;
			Confirmation Status:<b> <?php if ($own_row['birthPlace'] == '') echo 'Not Yet Confirmed';
											else if ($own_row['birthPlace'] == 'Yes') echo 'Confirmed'; else {echo $own_row['birthPlace'];}; ?></b>
		</div> 
		
		<br><br>
		<br><br>
	
	<?php if ($own_row['applicationStatus'] == 'Seat Allocated') { ?>
						...
						<h6><b>
								<font size="3">Congratulations, You are Provisionally admitted <?php
                                            ...
                                            <?php if ($own_row['title'] == 'HSC') {
                                                    ...
                                                    echo "<br><br> Last date for admission and uploading of joining report is 30-10-2021 failing which your seat will be cancelled automatically.";
	...
	<form id="seatForm" class="form-horizontal" role="form" method="get" enctype="multipart/form-data">
		<div class="container-fluid">
						...
						<table class="table table-bordered f11">
							<?php if ($user_row['modeOfAdmission'] == 'Through Centralised counselling') { ?>
								...
								<tr>
									...
									<?php if ($own_row['applicationStatus'] == 'Seat Allocated') { ?><td align="left"><a data-toggle="modal" data-target="#choicesTable" style="cursor:pointer;"><?php echo $user_row['collegeName'] . '(' . $user_row['collegeUniqueId'] . ')'; ?></a></td><?php } else {
								...
								<?php if ($user_row['applicationStatus'] == 'Seat Allocated') { ?>
									<tr>
			...
			<?php }?>
				<div class="container-fluid">
					<div class="col-sm-offset-1 col-sm-10" role="complementary">
						...
						<div class="well">
							...
							<h6><b>
									<font size="3">3. If you are not at all interested to take admission in the PMSSS Admission 2022-23 then click button (not interested in PMSSS).<br>
				...
				<button class="btn btn-success btn-block" type="button" data-toggle="modal" data-target="#iAgree" disabled>I Confirm the Seat (Freeze)</button>
					</div> 
				<!--<div class="col-lg-3">
					<button class="btn btn-info btn-block" type="button" data-toggle="modal" data-target="#optingForRound2" <?php if($own_row['applicationStatus']!='Seat Allocated') {echo 'disabled';} ?>>Opt for Next Round (Float)</button>
				</div>-->
				
				<div class="col-lg-2">
					<button class="btn btn-danger btn-block" type="button" data-toggle="modal" data-target="#notInterested" disabled>Not Interested in PMSSS</button>
				</div>
				...
				<p align="center"><br>
					<font color="red">
						<b>
							<?php if ($own_row['title'] == 'HSC') { ?>
								The last date for confirming the allotted seat is given in Calendar of events. If you don't select any of the 3 options your seat will be cancelled automatically.
							<?php } else { ?>
								The last date for confirming the allotted seat is Upto 25.10.2021. If you don't select any of the 3 options your seat will be cancelled automatically.
							<?php } ?>
						</b>
					</font>
				</p><br>
			...
			<input type="hidden" name="studentUniqueId" id="studentUniqueId" value="<?php echo $own_row['studentUniqueId']; ?>"></input>
		...
		$('#confirmSeat').click(function(e) {
			...
			if (($("#confirm").is(':checked'))) {
				...
				$.ajax({
					...
					url: 'partials/update/confirmAllottedSeat19Admin.php?button='+button+'&studentId='+studentUniqueId, // the url where we want to POST			
					encode: true,
					beforeSend: function() {
					...
					success: function(data) {
						...
						if (reply == "Success") {
							...
							setTimeout(function() {
								...
								alert('Seat Confirmed Successfuly');
								location.reload();								
							}, 4000);
						} else {
					...
					success: function(data) {
						...
						if (reply == "Success") {
							...
							setTimeout(function() {
								...
								alert('Seat Cancelled and Opted for Next Round.');
								location.reload();
							}, 4000);
						} else {
					...
					success: function(data) {
						...
						if (reply == "Success") {
							...
							setTimeout(function() {
								...
								alert('Marked as Not Interested in PMSSS successfully');
								location.reload();
							}, 4000);
						} else {
