...
$resideInCollege = $_POST['resideInCollegeHostel'];
$updateresideCollege = "update students set resideInCollege='" . $resideInCollege . "' where studentUniqueId='" . $studentUniqueId . "'";
$updateresult = mysqli_query($con, $updateresideCollege);
...
function uploadAttachments($studentUniqueId, $targetFile, $tmpName, $size, $imageNo, $db_path, $targetfile1) {
    ...
    if ($uploadOk !== "1") {
        ...
        if (move_uploaded_file($tmpName, $target_file)) {
            ...
            if ($imageNo === '1') {
                include("db_connect.php");
                $joinedOn = new DateTime($_POST['joiningDate'], new DateTimeZone('Asia/Kolkata'));
                $joinedOn = $joinedOn->format('Y-m-d');
                $joinQuery = "select name,yearOfCounselling,primaryEmailId,batch from students where studentUniqueId='" . $studentUniqueId . "'";
                $joinResult = mysqli_query($con, $joinQuery);
                $joinRow = mysqli_fetch_array($joinResult);
                if ($joinRow['batch'] == '2017-18' || $joinRow['batch'] == '2018-19') {
                    $queryx = "select joiningStatus from students_x where studentUniqueId='" . $studentUniqueId . "'";
                    $resultx = mysqli_query($con, $queryx);
                    $rowx = mysqli_fetch_array($resultx);
                    if ($rowx['joiningStatus'] == 'Not Accepted') {
                        $joiningReportQuery = "UPDATE students 
								SET joiningReport='" . $target_file1 . "',reported='Yes',joinedOn='$joinedOn'
								WHERE studentUniqueId='" . $studentUniqueId . "';";
                        $joiningReportQuery .= "UPDATE students_x 
								SET joiningComments=null,joiningStatus='Resubmitted'
								WHERE studentUniqueId='" . $studentUniqueId . "'";
                        $result = mysqli_multi_query($con, $joiningReportQuery) or die("Query Failed");
                    } else {
                        $joiningReportQuery = "UPDATE students 
								SET joiningReport='" . $target_file1 . "',reported='Yes',joinedOn='$joinedOn'
								WHERE studentUniqueId='" . $studentUniqueId . "'";
                    ...
                    include("mailer.php");
                    $recipients = $joinRow['primaryEmailId'];
                    $studentName = $joinRow['name'];
                    $subject = "Special Scholarship Scheme (PMSSS): (Uploaded Joining Report '" . $studentUniqueId . "')";
                    $body = "Dear '" . $studentName . "' ('" . $studentUniqueId . "'), <br/> <br/>			
					You have successfully uploaded your joining report. DBT option will be opened after verification of Joining report by AICTE. For claiming the scholarship refer <a href='https://www.aicte-india.org/sites/default/files/Final%204%20%20IMC_Methodology.pdf' target='_blank'>link</a>.<br/><br/>
					Regards,<br/>
					PMSSS Team<br/>
					<hr/>
