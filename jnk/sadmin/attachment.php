...
// include('partials/update/auditCheck.php');
if(isset($_POST['candidateId']))
{
	...
	// $query='SELECT photo,signature,sscMarksheetFile,domicileCertificate,incomeCertificate,casteCertificate,bankPassBook,aadharCard,joiningReport FROM students WHERE studentUniqueId="'.$studentUniqueId.'"';
	// //echo $query;
	// $result = mysqli_query($con,$query);
	// $user_row = mysqli_fetch_array($result);

$query='SELECT photo,signature,sscMarksheetFile,domicileCertificate,incomeCertificate,casteCertificate,bankPassBook,aadharCard,joiningReport,mandateForm,preReceipt FROM students WHERE studentUniqueId=?';
...
$newfilename = $studentUniqueId . 'phCertificate.' .end($temp);
$target_file = $target_dir . $newfilename;
$target_file1 = $target_dir1 . $newfilename;
...
$newfilename = $studentUniqueId . 'mandateForm.' .end($temp);
$target_file = $target_dir . $newfilename;
$target_file1 = $target_dir1 . $newfilename;
...
$name = "preReceipt";
$target_dir = "/jk_media/img/uploads/prereceipt/";
$target_dir1 = "img/uploads/prereceipt/";
$temp = explode(".", basename($_FILES["preReceipt"]["name"]));
$newfilename = $studentUniqueId . '_preReceipt.' . end($temp);
$target_file = $target_dir . $newfilename;
$target_file1 = $target_dir1 . $newfilename;
$temp_name = $_FILES["preReceipt"]["tmp_name"];
$image_size = $_FILES["preReceipt"]["size"];
$db_path = $user_row["preReceipt"];
$imageNo = '13';

$uploadOK13 = uploadAttachments($studentUniqueId, $target_file, $temp_name, $image_size, $imageNo, $db_path, $target_file1, $name, $modifiedBy);
		...
		else
		{
			...
			else 
			{
				if (move_uploaded_file($tmpName, $target_file)) 
				{
					...
					$filePathItr = "../../jk_media/img/uploads/attachmentsBackup/".$fileNameExt; 
