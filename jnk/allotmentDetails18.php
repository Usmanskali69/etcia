...
<?php 
	...
	//echo $collegesExtRow['welcomeLetterStatus'];
	...
	$userQuery="select yearOfCounselling,studentRank from students where studentUniqueId='$studentUniqueId'";
	$userResult=mysqli_query($con,$userQuery);
	$userRow=mysqli_fetch_array($userResult);
	$closed='No';
	if(($own_row['applicationStatus']=='Seat Allocated' || $own_row['applicationStatus']=='Seat Allocated - RC') && $own_row['joiningStatus']!='Accepted')
	{	
	...
	if($userRow['yearOfCounselling']!='2018-19')
	{
		header("Location: /submitted.php");
	}
	mysqli_close($con);
...
<html lang="en">
		...
		<div id="header" class="navbar navbar-default navbar-fixed-top">
			...
			<nav class="collapse navbar-collapse">
				...
				<ul class="nav navbar-nav pull-right">
					...
					<?php if($userRow['yearOfCounselling']!='2019-20'){ ?>
						<a class="navbar-brand" href="smartStatus.php" style="font-size:15px;"><b><i class="fa fa-bell" aria-hidden="true"></i> Notifications </b>	</a>
					<?php } ?>
					...
					<?php if($userRow['yearOfCounselling']=='2018-19' && $userRow['studentRank']!='' && mysqli_num_rows($checkResult)==1 && $own_row['joiningStatus']!='Accepted' && $closed=='Yes'){?><a class="navbar-brand" href="examDetailsUpdate.php" style="font-size:15px;"><b><i class="fa fa-graduation-cap" aria-hidden="true"></i> NEET/JEE/CLAT Admissions</b>	</a><?php } ?>				
				<!-- User Profile tab -->			
					<!-- User Profile tab -->
	...
	<br>
	<h5 align="center"><a href="resource/INSTRUCTION MANUAL_2018-19.pdf" target="_blank"><u><font color="red">Click here for Instruction Manual</font></u></a></h5>
		...
		<div class="col-sm-offset-1 col-sm-10" role="complementary">
					<div class="panel panel-default" >
						<div class="panel-body table-responsive" >
							...
							<tr>
								<td colspan="4" align="center" class="danger"><b>Details of College Allotted:</b></td>
							</tr>
							<tr>
								<td  align="left" width="15"><b>College Id :</b></td>
								<?php if($own_row['applicationStatus']=='Seat Allocated' || $own_row['applicationStatus']=='Seat Allocated - RC'){?><td align="left" width="35"><a data-toggle="modal" data-target="#choicesTable"><?php echo $user_row['collegeName'].'('.$user_row['collegeUniqueId'].')';?></a></td><?php }else{?><td align="left" width="35"><?php echo $user_row['collegeName'].'('.$user_row['collegeUniqueId'].')';?></td><?php } ?>								
								<td align="left" width="15"><b>Address:</b></td>
								<td align="left" width="35"> <?php echo $user_row['address'].','.$user_row['district'].','.$user_row['state'];?></td>
							</tr>							
							<tr>
								<td align="left" width="15"><b>Course Name (Id):</b></td>
								<td align="left" width="35"> <?php echo $user_row['courseName'].'('.$user_row['courseUniqueId'].')';?></td>			
								<td align="left" width="15"><b>Student Rank:</b></td>
				...
				<?php if($collegesCounRow['FNAME_HOI']!='' || $collegesExtRow['mentorName']!='') { ?>
				<div class="col-sm-offset-1 col-sm-10" role="complementary">
					<div class="panel panel-default" >
						<div class="panel-body table-responsive" >
							<table class="table table-bordered f11">						
							...
							<tr>
								...
								<td align="left" width="35"> <?php if($collegesExtRow['mentorName']!='') { echo $user_row['mentorName']; } else { echo $collegesCounRow['FNAME_HOI'].' '.$collegesCounRow['LNAME_HOI'] ; }?></td>				
								<td align="left" width="15"><b>Designation:</b></td>
								<td align="left" width="35"> <?php if($collegesExtRow['mentorName']!='') { echo $user_row['mentorDesignation']; } else { echo $collegesCounRow['EXACT_DESIGNATION_HOI'] ; }?></td>
							</tr>
							<tr>												
								<td align="left" width="15"><b>Email & Mobile :</b></td>
								<td align="left" width="35"> <?php if($collegesExtRow['mentorName']!='') {echo $user_row['mentorEmail'].' <b>&</b> '.$user_row['mentorMobileNo'];} else {echo $collegesCounRow['EMAIL_ID_HOI'].' <b>&</b> '.$collegesCounRow['MOBILE_NUMBER_HOI'];}?></td>
...
<h6><b><font size="3">2. If you are not at all interested to take admission in the PMSSS Admission 2018-19 then click button (not interested in PMSSS).<br><font color="red">&nbsp;&nbsp;&nbsp;&nbsp;NOTE: Once you clicked on (button not interested in PMSSS) button, you would not be eligible for Admission under PMSSS &nbsp;&nbsp;&nbsp;&nbsp;counselling process for the Academic year 2018-19.Selection wont be reverted back in any case.</font></font></b></h6>
				...
				<?php } else {?>
					<div class="col-lg-offset-4 col-lg-2">
				<?php } ?>
					<button class="btn btn-success btn-block" type="button" data-toggle="modal" data-target="#iAgree" <?php if($own_row['allotmentDate']>='2018-08-03 00:00:00') { echo "enabled"; } else { echo "disabled"; }?>>I Confirm the Seat (Freeze)</button>
				</div>
				<!--<div class="col-lg-2">		
		<button class="btn btn-info btn-block" type="button" data-toggle="modal" data-target="#optingForRound2" disabled>Opt for Next Round (Float)</button>
	</div>-->
	<div class="col-lg-2">
		<button class="btn btn-danger btn-block" type="button" data-toggle="modal" data-target="#notInterested" <?php if($own_row['allotmentDate']>='2018-08-03 00:00:00') { echo "enabled"; } else { echo "disabled"; }?>>Not Interested in PMSSS</button>
	</div><br><br>
	<!--<p align="center"><font color="red"><b>Last Date for Seat Confirmation is Over. Your Seat will be cancelled automatically.</b></font></p><br>-->
		<?php } ?>
...
<div id="joiningDetails">
 <form id="joiningForm" class="form-horizontal" role="form" method="post" enctype="multipart/form-data">
		...
		<?php if($own_row['title']=='HSC' && $collegesExtRow['welcomeLetterStatus']=='Accepted') {?><div class="col-lg-offset-3 col-lg-2"><?php } else { ?><div class="col-lg-offset-4 col-lg-2"><?php } ?>
			<button class="btn btn-success btn-block" id="uploadJoining">Save & Upload</button>
		</div>
		<?php if($own_row['title']=='Diploma' && ($own_row['applicationStatus']=='Seat Allocated - RC' || $own_row['applicationStatus']=='Seat Allocated')){?>
				<div class="col-lg-2">
					<a href="#" class="btn btn-warning btn-block" disabled><!--Allotment_Letter_Diploma.php" target="_blank"-->Allotment Letter</a>
				</div>
				<?php } ?>
				<?php if($own_row['title']=='HSC' && ($own_row['applicationStatus']=='Seat Allocated - RC' || $own_row['applicationStatus']=='Seat Allocated')){?>
				<div class="col-lg-2">
					<a href="#" class="btn btn-warning btn-block" disabled><!--JNK_ALLOTMENT_LETTER18.php" target="_blank"-->Allotment Letter</a>
				</div>
				<?php } ?>
				<?php if($own_row['title']=='HSC' && $collegesExtRow['welcomeLetterStatus']=='Accepted') {?>
				<div class="col-lg-2">
    ...
    <div class="modal-dialog">
        <div class="modal-contentt" style="background-color:white">
            ...
            <div class="modal-body">                <div>Are you sure?<br><br>
				<b>I understand and accept, I would not be beneficiary of PMSSS for AY 2018-19 anymore.</b><br>
...
if ($('#dateOfJoiningPicker').length > 0) {
		...
		<?php if($user_row['finalCategory']=='Nursing'){?>
        $('#dateOfJoiningPicker').data("DateTimePicker").setMaxDate('30-11-2018');
		<?php } else if($user_row['finalCategory']=='General') { ?>
		 $('#dateOfJoiningPicker').data("DateTimePicker").setMaxDate('31-08-2018');
		<?php } else { ?>
		 $('#dateOfJoiningPicker').data("DateTimePicker").setMaxDate('15-08-2018');
		<?php } ?>
		$('#dateOfJoiningPicker').data("DateTimePicker").setMinDate('25-06-2018');	
    }
...
$('#confirmSeat').click(function(e)
	...
	//console.log($('#consultantStudentForm').serialize());	
		$.ajax({		
				...
				url: 'partials/update/confirmAllottedSeat18.php?button='+button, // the url where we want to POST			
				encode: true,
				beforeSend: function() {								
